...
def extract_facts(ami: str, working_directory: Path) -> bool:
...
def upload_and_run_script(ami: str, script: Path):
    if not script.exists():
        LOGGER.error(f"{script} does not exist.")
        return

    with DigitalOceanWrapper(ami) as aw:
        aw.wait_for_instance()
...
def handle_aws(args):
    ...
    if args.s:
        upload_and_run_script(ami, Path(args.s))
        return
    ...
    try:
        facts_extracted = extract_facts(ami, temp_dir)
    except Exception as e:
